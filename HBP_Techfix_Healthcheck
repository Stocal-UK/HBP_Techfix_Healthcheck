...
$bootDriveLetter = (Get-WmiObject -Class Win32_OperatingSystem).SystemDrive
$bootDisk = Get-Partition -DriveLetter $bootDriveLetter.TrimEnd(":\") | Get-Disk
$mediaType = $bootDisk.MediaType

if ($mediaType -eq "SSD") {
    $SSD = 'Y'
    Write-Host "Boot Drive: SSD" -ForegroundColor Green
}
elseif ($mediaType -eq "HDD") {
    $SSD = 'N'
    Write-Host "Boot Drive: HDD" -ForegroundColor Green
}
else {
    $SSD = 'N'
    Write-Host "Boot Drive: Unknown MediaType" -ForegroundColor Green
}
